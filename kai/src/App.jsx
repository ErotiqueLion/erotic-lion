...
4. 必ず指定のJSON形式（valid, feedback, word, word_reading, next_kana, arousal_inc, player_lost, sister_lost, tts_instruction）のみを返してください。`;
      ...
      // (C-2) AI返答の読みが正しい先頭文字から始まっているか検証
      //       kuroshiro が使える場合はAI単語の読みもクライアント側で取得して照合
      let aiWordReading = result.word_reading ? toHiragana(result.word_reading) : null;
      if (result.word && kuroshiroReady) {
        const kuroAiReading = await getReading(result.word);
        if (kuroAiReading) aiWordReading = toHiragana(kuroAiReading);
      }
      ...
      if (readingFirst && readingFirst !== expectedNextStart && !result.player_lost && !result.sister_lost) {
        ...
        try {
          ...
          if (retryRaw) {
            ...
            let retryReading = retryResult.word_reading ? toHiragana(retryResult.word_reading) : null;
            if (retryResult.word && kuroshiroReady) {
              const kr = await getReading(retryResult.word);
              if (kr) retryReading = toHiragana(kr);
            }
            const retryFirst = retryReading
              ? (smallToLargeMap[retryReading.charAt(0)] || retryReading.charAt(0))
              : null;
            if (retryFirst === expectedNextStart && !s.history.includes(retryResult.word)) {
              Object.assign(result, retryResult);
              if (retryReading) result.word_reading = retryReading;
            }
          }
        } catch (retryErr) {
      ...
      // (C-3) AI返答の読み末尾が「ん」なら AI の負け（kuroshiro 読みで判定）
      const aiReadingFinal = aiWordReading || (result.word_reading ? toHiragana(result.word_reading) : null);
      if (aiReadingFinal && aiReadingFinal.slice(-1) === 'ん') {
        result.sister_lost = true;
